...
class Product extends Entity {
    ...
    public function getForOrderSupplier($order = 0) {
        ...
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        if ($order > 0) {
            $sql11 = "select ediorder from edi_order_item where porder = '" . $order . "' AND  ediitem in (SELECT id FROM partsbox_db.edi_item where edi = 11 AND replace(replace(replace(replace(`partno`, '/', ''), '.', ''), '-', ''), ' ', '') LIKE '" . $this->getItemCode2() . "')";
            $connection = $em->getConnection();
