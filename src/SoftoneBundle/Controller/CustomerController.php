...
class CustomerController extends \SoftoneBundle\Controller\SoftoneController {
    ...
    public function autocompletesearch() {
        ...
        $query = $em->createQuery(
                        "SELECT p.id, p.customerName,p.customerAfm,p.customerCode,p.status FROM " . $this->repository . " " . $this->prefix . " where p.vin LIKE '%" . $_GET["term"] . "%' OR p.pinakida LIKE '%" . $_GET["term"] . "%' OR p.customerName LIKE '%" . $_GET["term"] . "%' OR p.customerPhone01 LIKE '%" . $_GET["term"] . "%' OR p.customerPhone02 LIKE '%" . $_GET["term"] . "%' OR p.customerAfm LIKE '%" . $_GET["term"] . "%' OR p.customerCode LIKE '%" . $_GET["term"] . "%'"
                )
                ->setMaxResults(20)
                ->setFirstResult(0);
    ...
    public function gettabs($id) {
        ...
        if ($id == 0 AND @ $entity->id == 0) {
            ...
            if ($this->getSetting("SoftoneBundle:Softone:apothiki") == 'carparts') 
                //$this->setSetting("SoftoneBundle:Customer:customerCode", "1".$customerCode);   
                $entity->setField("customerCode", str_pad("1".$customerCode, 7, "0", STR_PAD_LEFT));
            else 
                $this->setSetting("SoftoneBundle:Customer:customerCode", $customerCode);
        ...
        $fields["pinakida"] = array("label" => $this->getTranslation("Πινακίδα"), "className" => "col-md-6", "required" => false);
        $fields["vin"] = array("label" => $this->getTranslation("Αριθμός Πλαισίου"), "className" => "col-md-6", "required" => false);
    ...
    function retrieveCustomer() {
        ...
        $where = "AND M.UPDDATE >= '" . date("Y-m-d", strtotime("-1 days")) . "' AND COMPANY = ".$company;
    ...
    function retrieve($params = array()) {
        ...
        if ($this->getSetting("SoftoneBundle:Softone:merchant") == 'foxline') {
            ...
            $sql = "update `softone_customer` SET `store` = '1' WHERE `store` IS NULL";
            $this->getDoctrine()->getConnection()->exec($sql);
        }
        if ($this->getSetting("SoftoneBundle:Softone:apothiki") == 'carparts') {
            $sql = "update `softone_customer` set `price_field` = 'itemPricew' WHERE `price_field` IS NULL";
            $this->getDoctrine()->getConnection()->exec($sql);
            $sql = 'update `softone_customer` set customergroup = 1 where customergroup is null';
            $this->getDoctrine()->getConnection()->exec($sql);            
        }
