        ...
        foreach ($fields as $field => $options) {
            ...
            if ($options["type"] == 'select') {
                ...
                if ($options["datasource"]) {
                    ...
                    $seloptions[] = array("name" => "Select", "value" => 0);                    
                ...
                @$forms["fields"][] = array("key" => $field, "className" => (string) $options["className"], "id" => $this->repository . ":" . $field . ":" . $entity->getId(), "defaultValue" => $entity->getField($field), "type" => $options["type"], "templateOptions" => array("type" => '', 'disabled'=>$options["disabled"], 'class' => '', "label" => $options["label"], "required" => $options["required"]));
            }
        }
        return $forms;
    }
            ...
            if ($items) {
                ...
                foreach ($items as $term) {
                    ...
                    if ($category > 0) {
                        ...
                        $sql = "SELECT art_id, art_article_nr_can,sup_id,sup_brand FROM `articles`,suppliers where sup_id = art_sup_id AND `art_id` in (SELECT `art_id` FROM magento2_base4q2017.articles art WHERE (art.art_id in (SELECT art_id FROM `art_oem_numbers` WHERE `oem_num_can` LIKE '" . $term . "' ".$brand_sql.")))  " . $sup . "";
                        //echo $sql;
                    }
                    //$sql = "SELECT art_article_nr_can,sup_id,sup_brand FROM `articles`,suppliers where sup_id = art_sup_id AND art_article_nr_can in ('".implode("','",$items)."') order by sup_brand";
                    //echo $sql;
