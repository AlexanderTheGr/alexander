    ...
    function getFormLyFields($entity, $fields, $id = '') {
        ...
        foreach ($fields as $field => $options) {
            ...
            if ($options["type"] == 'select') {
                ...
                @$forms["fields"][] = array("key" => $field, "id" => $this->repository . ":" . $field . ":" . $entity->getId(), 'defaultValue' => $defaultValue, "type" => "select", "templateOptions" => array("type" => '', 'options' => $seloptions, 'defaultOptions' => array("value" => $defaultValue), "label" => $options["label"], "required" => 0/*$options["required"]*/));
            } else {
                ...
                @$forms["fields"][] = array("key" => $field, "className" => (string)$options["className"], "id" => $this->repository . ":" . $field . ":" . $entity->getId(), "defaultValue" => $entity->getField($field), "type" => "input", "templateOptions" => array("type" => '', 'class' => '', "label" => $options["label"], "required" => 0/*$options["required"]*/));
            }
        }
        return $forms;
    }
