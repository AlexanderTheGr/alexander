...
class EdiItemController extends Main {
    ...
    public function ediitemdatatable($funct = false, $id = false) {
        ...
        foreach (@(array) $results as $result) {
            ...
            $sql = "Select id from megasoft_product where replace(replace(replace(replace(replace(`supref`, '/', ''), '.', ''), '-', ''), ' ', ''), '*', '')  = '" . $this->clearstring($obj->getItemCode()) . "' AND item_mtrsup = '" . $obj->getEdi()->getId() . "'";
            //echo $sql . "<BR>";
