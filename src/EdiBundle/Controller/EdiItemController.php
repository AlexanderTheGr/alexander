...
class EdiItemController extends Main {
    ...
    public function autocompletesearchAction($edi) {
        ...
        $query = $em->createQuery(
                        "SELECT p.id, p.itemCode,p.partno,p.artNr,p.artNr,p.description, p.brand FROM " . $this->repository . " " . $this->prefix . " where p.Edi=".$entity->getEdi()." AND (p.partno LIKE '" . $_GET["term"] . "' OR p.itemCode LIKE '%" . $_GET["term"] . "%' OR p.artNr LIKE '" . $_GET["term"] . "%')"
                )
                ->setMaxResults(20)
                ->setFirstResult(0);
